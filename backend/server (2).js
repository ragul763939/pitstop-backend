...
// Root — status page (fixes "Cannot GET /")
app.get('/', (req, res) => {
  res.send(`<!DOCTYPE html><html><head><meta charset="UTF-8"/>
<title>PitStop API</title>
<style>
  body{font-family:system-ui,sans-serif;background:#0A0A0A;color:#F5F0E8;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0}
  .box{background:#161616;border:1px solid rgba(255,255,255,0.08);border-radius:16px;padding:40px 48px;max-width:500px;width:90%}
  h1{font-size:28px;margin:0 0 4px;color:#FF4D00}
  ...
  .green{color:#00C47D}.badge{display:inline-block;background:rgba(0,196,125,0.12);color:#00C47D;padding:3px 10px;border-radius:6px;font-size:12px;margin-bottom:24px}
  .ep{background:#111;border-radius:10px;padding:16px;margin-top:20px;font-size:13px;color:#888;line-height:1.8}
  .ep span{color:#FF4D00}
</style></head><body><div class="box">
  <h1>⛽ PitStop API</h1>
  <div class="sub">India's first on-demand fuel & mechanic platform</div>
  <div class="badge">● Server is running</div>
  <div class="row"><span class="k">Status</span><span class="v green">Online ✓</span></div>
  <div class="row"><span class="k">Version</span><span class="v">1.0.0</span></div>
  <div class="row"><span class="k">Time</span><span class="v">${new Date().toLocaleString('en-IN')}</span></div>
  <div class="row"><span class="k">Vendors loaded</span><span class="v">${DB.vendors.length} active</span></div>
  <div class="row"><span class="k">Orders today</span><span class="v">${DB.orders.length}</span></div>
  <div class="ep">
    <span>GET</span>  /health — JSON health check<br>
    <span>GET</span>  /api/vendors — list vendors<br>
    <span>GET</span>  /api/price?fuel=petrol&amp;litres=5&amp;distance=3<br>
    <span>POST</span> /api/orders — place order<br>
    <span>GET</span>  /api/orders/:id — track order<br>
    <span>POST</span> /api/sos — emergency SOS
  </div>
</div></body></html>`);
});

// Health
app.get('/health', (req,res) => res.json({ status:'ok', time:new Date().toISOString(), version:'1.0.0', vendors:DB.vendors.length, orders:DB.orders.length }));
...
server.listen(PORT, () => {
  console.log(`\n🚀 PitStop API → http://localhost:${PORT}`);
  console.log(`   Socket.io : enabled`);
  console.log(`   Vendors   : ${DB.vendors.length} pre-loaded\n`);
});
